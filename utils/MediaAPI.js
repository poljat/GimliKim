...
const getProfilePic = (profId) => {
  return fetch('http://media.mw.metropolia.fi/wbma/tags/profile').
      then(response => {
        return response.json();
      }).
      then(profPic => {
...
const getJoinedChats = async () => {
  ...
  return fetch(url + 'favourites', settings).then(res => {
    return res.json()
  }).then(json => {
    return Promise.all(json.map(chat => {
      return fetch(url + "media/" + chat.file_id).then(res => {
        return res.json();
      });
    })).then(chats => {
      return chats;
